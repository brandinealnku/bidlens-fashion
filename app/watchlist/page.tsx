...
export default async function Page() {
  ...
  const initial = records.map((record) => ({
    ...
    expectedProfit:record.listing.recommendations[0]?.expectedNetProfitAtCurrentBid??0,
    expectedRoi:record.listing.recommendations[0]?.expectedROIAtCurrentBid??null,
    auctionEndAt:record.listing.auctionEndAt?.toISOString()??null,
    userMaximumBid:record.userMaximumBid,plannedBid:record.plannedBid,priority:record.priority,decisionDeadline:record.decisionDeadline?.toISOString().slice(0,10)??'',notes:record.notes??'',
  }));
  return (
    <>
      <h1>Watchlist</h1>
