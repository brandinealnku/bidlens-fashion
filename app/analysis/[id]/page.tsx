...
async function saveScenario(data: FormData) {
  'use server';
  const user = await requireCurrentUser(),
    auctionListingId = String(data.get('listingId')),
    name = String(data.get('name'));
  const listing = await db.auctionListing.findFirst({
    where: { id: auctionListingId, userId: user.id },
  });
  if (!listing) throw new Error('Listing not found');
  await db.$transaction([
    db.savedScenario.updateMany({
      where: { userId: user.id, auctionListingId },
      data: { isActive: false },
    }),
    db.savedScenario.upsert({
      where: {
        userId_auctionListingId_name: {
          userId: user.id,
          auctionListingId,
          name,
        },
      ...
      create: {
        ...
        auctionListingId,
        name,
        resaleValueCents: Number(data.get('resale')),
        sellingFeeBasisPoints: Number(data.get('fee')),
        shippingCents: Number(data.get('shipping')),
        cleaningRepairCents: Number(data.get('cleaning')),
        authenticationCents: Number(data.get('authentication')),
        desiredProfitCents: Number(data.get('profit')),
        isActive: true,
      },
      update: {
        isActive: true,
  ...
  revalidatePath(`/analysis/${auctionListingId}`);
}
...
export default async function Analysis({ params }: { params: { id: string } }) {
  ...
  const scenarioRows = bid
    ? [
        [
          'Conservative',
          valuation?.quickSaleEstimate ?? bid.expectedResaleValue,
          1600,
          3000,
          3000,
          5000,
        ...
        [
          'Expected',
          bid.expectedResaleValue,
          1300,
          bid.outboundShipping,
          bid.cleaningAllowance,
          bid.authenticationAllowance,
        ],
        [
          'Optimistic',
          valuation?.optimisticResaleValue ?? bid.expectedResaleValue,
          1100,
          bid.outboundShipping,
          0,
          0,
        ...
        result: calculateScenario({
          resaleValueCents: Number(resale),
          purchasePriceCents: listing.currentBid,
          sellingFeeBasisPoints: Number(fee),
          shippingCents: Number(shipping),
          cleaningRepairCents: Number(cleaning),
          authenticationCents: Number(authentication),
          desiredProfitCents: bid.minimumRequiredProfit,
        }),
      }))
    : [];
  return (
    <>
      ...
      <section className="card opps table-wrap">
        ...
        <p className="muted">
          Every scenario uses the same cent-based calculation utility; only
          assumptions change.
        </p>
        {scenarioRows.length ? (
          <table>
            ...
            <tbody>
              {scenarioRows.map((x) => (
                <tr key={x.name}>
                  <td>
                    <b>{x.name}</b>
                    {listing.scenarios.find((s) => s.name === x.name)
                      ?.isActive && <span className="badge BUY">Active</span>}
                  </td>
                  <td>{money(x.resale)}</td>
                  <td>{money(x.result.totalCostCents)}</td>
                  <td>{money(x.result.profitCents)}</td>
                  <td>
                    {x.result.roiBasisPoints === null
                      ? '—'
                      : `${(x.result.roiBasisPoints / 100).toFixed(1)}%`}
                  </td>
                  <td>{money(x.result.maximumBidCents)}</td>
                  <td>
                    <form action={saveScenario}>
                      ...
                      <input type="hidden" name="resale" value={x.resale} />
                      <input type="hidden" name="fee" value={x.fee} />
                      <input type="hidden" name="shipping" value={x.shipping} />
                      <input type="hidden" name="cleaning" value={x.cleaning} />
                      <input
                        type="hidden"
                        name="authentication"
                      ...
                      <input
                        type="hidden"
                        name="profit"
                        value={bid?.minimumRequiredProfit ?? 0}
                      />
          ...
          <p>Run valuation to compare scenarios.</p>
        )}
      </section>
      <section className="card opps">
