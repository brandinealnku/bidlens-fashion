...
export function scoreOpportunity(item: Opportunity, profile: Profile) {
  ...
  if (headroom > 0) {
    marketScore += Math.min(
      20,
      Math.round((headroom / Math.max(item.maximumBidCents, 1)) * 40),
    );
  ...
  if ((item.expectedRoiBasisPoints ?? -1) >= profile.minimumRoiBasisPoints)
    personalScore += 10;
  ...
  return {
    ...
    recommendedNextAction:
      personalScore >= 75 && headroom > 0
        ? 'Add to watchlist and plan a bid'
        : personalScore >= 45
          ? 'Review assumptions'
          : 'Pass',
  };
...
export function calculateScenario(input: {
  ...
  sellingFeeBasisPoints: number;
  shippingCents: number;
  cleaningRepairCents: number;
  authenticationCents: number;
  desiredProfitCents: number;
}) {
  const sellingFeesCents = Math.round(
    (input.resaleValueCents * input.sellingFeeBasisPoints) / 10000,
  );
  const nonPurchaseCostsCents =
    sellingFeesCents +
    input.shippingCents +
    input.cleaningRepairCents +
    input.authenticationCents;
  ...
  return {
    totalCostCents,
    profitCents,
    roiBasisPoints: totalCostCents
      ? Math.round((profitCents * 10000) / totalCostCents)
      : null,
    maximumBidCents: Math.max(
      0,
      input.resaleValueCents - nonPurchaseCostsCents - input.desiredProfitCents,
    ),
  };
}
...
export function accuracyMetrics(
  ...
  const resaleErrors = values.map((x) =>
    Math.abs(x.actualCents - x.predictedCents),
  );
  const profitErrors = values.map(
    (x) => x.actualProfitCents - x.predictedProfitCents,
  );
  const rate = (predicate: (x: (typeof values)[number]) => boolean) =>
    values.length
      ? Math.round((values.filter(predicate).length * 10000) / values.length)
      : 0;
  return {
    count: values.length,
    meanAbsoluteResaleValueErrorCents: values.length
      ? Math.round(resaleErrors.reduce((a, b) => a + b, 0) / values.length)
      : 0,
    medianAbsoluteResaleValueErrorCents: median(resaleErrors),
    meanProfitErrorCents: values.length
      ? Math.round(profitErrors.reduce((a, b) => a + b, 0) / values.length)
      : 0,
    medianProfitErrorCents: median(profitErrors),
    accuracyBasisPoints: rate(
      (x) =>
        Math.abs(x.actualCents - x.predictedCents) * 10000 <=
        Math.max(1, x.predictedCents) * toleranceBasisPoints,
    ),
    profitableBasisPoints: rate((x) => x.actualProfitCents > 0),
    maximumBidComplianceBasisPoints: rate(
      (x) => x.purchasePriceCents <= x.maximumBidCents,
    ),
    averageHoldingPeriodDays: values.length
      ? Math.round(values.reduce((s, x) => s + x.daysHeld, 0) / values.length)
      : 0,
  };
}

export const inventoryTransitions: Record<string, string[]> = {
  AWAITING_PAYMENT: ['AWAITING_SHIPMENT', 'RETURNED'],
  AWAITING_SHIPMENT: ['IN_TRANSIT', 'RETURNED'],
  ...
  AUTHENTICATION_REQUIRED: ['CLEANING_REQUIRED', 'READY_TO_LIST', 'RETURNED'],
  CLEANING_REQUIRED: ['READY_TO_LIST', 'RETURNED'],
  READY_TO_LIST: ['LISTED'],
  LISTED: ['SOLD', 'RETURNED'],
  SOLD: ['CLOSED'],
  RETURNED: ['CLOSED'],
  CLOSED: [],
};
export const canTransitionInventory = (from: string, to: string) =>
  inventoryTransitions[from]?.includes(to) ?? false;
